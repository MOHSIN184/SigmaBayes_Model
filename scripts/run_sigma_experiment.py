...
def train_sigma_cnn(config, train_loader, val_loader, train_split, model_path, history_path):
    print("\nTraining sigma CNN with mandatory class weights...")
    device = get_device()
...
def evaluate_test_set(config, model, test_loader, figure_dir, table_dir, device):
    print("\nEvaluating on untouched sigma test set...")
    ...
    save_metrics_table(metrics, table_dir / "sigma_metrics.csv")
    save_classification_report(
        test_labels,
        test_pred,
        list(config.class_names),
        table_dir / "sigma_classification_report.csv",
    )
    plot_confusion_matrix(
        test_labels,
        test_pred,
        list(config.class_names),
        save_path=figure_dir / "sigma_confusion_matrix.png",
    )
...
def run_mc_dropout(config, model, test_loader, figure_dir, table_dir, device):
    ...
    uncertainty_df = save_uncertainty_results(
        mc_result,
        list(config.class_names),
        table_dir / "sigma_uncertainty_results.csv",
    )
    plot_uncertainty_histogram(
        uncertainty_df,
        save_path=figure_dir / "sigma_uncertainty_histogram.png",
        column="entropy",
    )

    uncertainty_summary = create_uncertainty_by_class_summary(uncertainty_df)
    uncertainty_summary.to_csv(table_dir / "sigma_uncertainty_by_class.csv", index=False)
    plot_uncertainty_by_class(
        uncertainty_summary, figure_dir / "sigma_uncertainty_by_class.png"
    )
...
def run_temperature_scaling(
    ...
    plot_reliability_diagram(
        ...
        save_path=figure_dir / "sigma_reliability_before.png",
        n_bins=15,
        title="Sigma Reliability Before Temperature Scaling",
    )
    plot_reliability_diagram(
        test_labels_from_loader,
        calibration_result["test_proba_after"],
        save_path=figure_dir / "sigma_reliability_after.png",
        n_bins=15,
        title="Sigma Reliability After Temperature Scaling",
    )
    save_calibration_metrics(
        calibration_result["before"],
        calibration_result["after"],
        calibration_result["temperature"],
        table_dir / "sigma_calibration_metrics.csv",
    )
    save_calibrated_probabilities(
        test_labels,
        calibration_result["test_proba_before"],
        calibration_result["test_proba_after"],
        table_dir / "sigma_calibrated_probabilities.csv",
    )
...
def run_conformal(
    ...
    for alpha, suffix in [(0.1, "90"), (0.05, "95")]:
        ...
        save_conformal_results(
            ...
            test_labels,
            calibrated_test_proba,
            table_dir / f"sigma_conformal_predictions_{suffix}.csv",
            class_names=list(config.class_names),
        )
    ...
    save_conformal_metrics(results_by_alpha, table_dir / "sigma_conformal_metrics.csv")

    classwise_summary = create_conformal_classwise_summary(
        config, test_labels, outputs[0.1], outputs[0.05]
    )
    classwise_summary.to_csv(
        table_dir / "sigma_conformal_classwise_summary.csv", index=False
    )
...
def run_experiment(config=None):
    ...
    print("\nSigma multi-class experiment completed successfully.")
